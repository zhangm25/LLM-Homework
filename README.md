...
RoamMind 是一个面向真实出行场景的对话式行程规划原型：它不要求用户先填一张复杂表单，而是从一句自然语言开始，理解“从哪里出发、想做什么、最后回哪儿”，并把模糊需求逐步落到地图上的真实地点。用户可以说“有点累，想找个安静的地方坐坐，顺便吃点东西”，也可以直接上传行程表、会议安排或旅行计划，系统会把地点、时间、偏好和约束整理成可执行路线。

它的核心不是简单生成一段旅行文案，而是把大模型的语义理解、高德地图的真实 POI 检索、候选地点确认、路线耗时估算和时间轴排程串在一起。对于“附近吃饭”“顺路买东西”“回到宿舍”这类容易出错的表达，RoamMind 会先确定地点，再规划移动；地点不明确时给出候选卡片和地图点选，用户换一个候选后路线会自动重算。地图也支持放大查看，适合在候选地点较多时做空间判断。

一句话概括：RoamMind 试图把“和人说行程”的轻松感，和“地图上真的能走”的可靠性接到一起。
...
- ✅ 已实现：意图理解（情绪/清晰两类卡片）、**非行程/矛盾/空输入主动澄清**（复用 LLM 自带追问）、
  ...
  **地点优先确认流程**（候选卡片、地图点选、候选切换后确定性重算）、
  时间轴排程、地图可视化（高德 JS / SVG 兜底，支持地图放大查看）、流式对话、一键唤起导航（深链 + Web 兜底）、
  **附件行程表解析**、**LLM 超时/异常自动降级规则抽取**、全链路无 Key 降级、回归测试 `backend/tests/test_roammind.py`（58 项）。
